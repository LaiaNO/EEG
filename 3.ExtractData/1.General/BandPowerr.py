...
def plotbar_info_bandpower1(names, sorted_list_EO, TeiQueSF_emotionality, list_bands):
    ...
    minus5 = return_bandpower(names, sorted_list_EO,
                              TeiQueSF_emotionality, list_bands, 'Min')
    max5 = return_bandpower(names, sorted_list_EO, TeiQueSF_emotionality,
                            list_bands, 'Max')  # the median of the max 4

    str1 = return_str(names, sorted_list_EO, TeiQueSF_emotionality,
                      list_bands, 'Max')  # standar error of the mean
    str2 = return_str(names, sorted_list_EO, TeiQueSF_emotionality,
                      list_bands, 'Min')  # standar error of the mean

    x = np.arange(len(labels))  # the label locations
    width = 0.35  # the width of the bars

    fig, ax = plt.subplots()
    ax.bar(x - width/2, minus5, width, yerr=str2, capsize=0.05, label='-4')
    ax.bar(x + width/2, max5, width, yerr=str1, capsize=0.05, label='+4')
...
def plotbar_info_rac(names, sorted_list_EO, TeiQueSF_emotionality, list_bands):
    ...
    max4 = return_rac(names, sorted_list_EO,
                      TeiQueSF_emotionality, list_bands, 'Max')
