    ...
    for e in range(0,len(pacientData)): #For each frequency band:
        ...
        for i in range(0, len(patientNames)): #For each patient:
            ...
            if hename in testNames:   #If this is in the testNames:
                ...
                g = (float(gender[int(indices[0])])) #Get the gender of the patient 
                if str(g)=='1.0':
                    xCorr_f.append(x)
        ...
        savecor_f.append(stats.pearsonr(xCorr_f, yCorr_f)) #0: Pearson’s correlation coefficient, 1: Two-tailed p-value

        print(stats.pearsonr(xCorr_m, yCorr_m)) #Print the stats.pearsonr
        savecor_m.append(stats.pearsonr(xCorr_m, yCorr_m)) #0: Pearson’s correlation coefficient, 1: Two-tailed p-value
    ...
    for e in pacientData: #For each frequency band:
        ...
        for i in range(0, len(patientNames)): #For each patient:
            ...
            if hename in testNames:   #If this is in the testNames:
                ...
                g = (float(gender[int(indices[0])])) #Get the gender of the patient 
                if minMax == 'Max':
                    if str(g)=='1.0' and x>4.0:
                        y = (float(e[i]))
                        y_corr_f.append(y)
                    if str(g)=='2.0' and x>4.0:
                        y = (float(e[i]))
                        y_corr_m.append(y)
                if minMax == 'Min':
                    if str(g)=='1.0' and x<4.0:
...
def plotbar_info_bandpower1 (names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands):
    ...
    minus5_f, minus5_m = return_bandpower1(names, sorted_list_EO, TeiQueSF_emotionality, gender,list_bands, 'Min')
    max_5_f, max_5_m = return_bandpower1(names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands, 'Max')
    #print(minus5)
    strfMax, strmMax = return_str1(names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands, 'Max')
    strfMin, strmMin = return_str1(names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands, 'Min')
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x-0.2, minus5_f, width, yerr=strfMin,capsize=0.05,label='-4 women')
    rects3 = ax.bar(x, max_5_f, width, yerr=strfMax,capsize=0.05, label='+4 women')

    rects2 = ax.bar(x+0.2, minus5_m, width, yerr=strmMin,capsize=0.05,label='-4 men')
    rects4 = ax.bar(x+0.4, max_5_m, width, yerr=strmMax,capsize=0.05, label='+4 men')
...
def plotbar_info_rac (names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands):
    ...
    minusF, minusM = return_rac(names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands, 'Min')
    maxF, maxM = return_rac(names, sorted_list_EO, TeiQueSF_emotionality, gender, list_bands, 'Max')
